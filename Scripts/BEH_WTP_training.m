function [responses, timing, windowHandle] = BEH_WTP_training(subject_id, session, folders, nTrials, windowHandle)

% This function allows to practice a given number of trials of the WTP task
%%%% Inputs %%%%
...
% 'nTrials' = number of trials (integer). Ex. : 3. Please be aware the nTrials should be smaller than or
% equal to the number of available example images (by default, nTrials = 3).
...
% Check variables
if  exist('subject_id', 'var') && exist('session', 'var') && exist('folders', 'var') && exist('nTrials', 'var')
    if ischar(subject_id) && ischar(session) && isstruct(folders) && isnumeric(nTrials)
        % Output file
        responses_file_name = [folders.write_folder filesep 'SNS_BEH_WTP_training_S', subject_id, '_', session, '.mat'];
...
imageMatrix = cell(nTrials, 1); % Example lottery images
for currImage = 1:nTrials
    imageMatrix{currImage, 1} = double(imread([folders.examples_folder filesep 'example_' num2str(currImage) '.png'])); 
end
...
try
    ...
    if all(isnan(responses.maxPrice))
        DrawFormattedText(windowHandle, 'Please raise your hand', 'center', 'center', colors.textColor, [], [], [], 2);
        Screen('Flip', windowHandle);
